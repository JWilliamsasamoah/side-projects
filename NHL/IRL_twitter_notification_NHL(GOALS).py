...
url = f'https://nhl-{confDict[homeTeam]}-rest.oasis.smt.com/optics3d/v2.80/MarkerActivity?Game={gameID}&MinorType=EventGoal'
response = requests.get(url, auth=connect) 
print(response)
...
GOAL=scoreboardData[scoreboardData['Confidence']==75]
HITS=scoreboardData[scoreboardData['Confidence']==100]
GOAL['Descriptor_']
...
# other variables by getting the keys and the token you can find your keys and tokens on Twitter api devlopper 
  while True:
        ...
        print(GOAL['Descriptor_'].tail(1))
        tweet = {"text":GOAL['Descriptor_'].iloc[-1]}
